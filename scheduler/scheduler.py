...
class JobScheduler:
    ...
    _find_project_root(script_path):
        Sucht in den Elternverzeichnissen von script_path nach einem Projekt-Root-Marker
        (.git, pyproject.toml, setup.py, setup.cfg, requirements.txt) und gibt ihn zurück.
    _run_script(script_path, config_path, venv_path=None, working_dir=None):
        Führt das angegebene Skript mit optionaler Konfiguration und Virtual-Environment aus.
        working_dir überschreibt das Standard-cwd (script_path.parent).
        PYTHONPATH wird automatisch um das effektive Arbeitsverzeichnis und den erkannten
        Projekt-Root (falls vorhanden) erweitert.
    run_pending():
    ...
    def _find_project_root(self, script_path: Path) -> Optional[Path]:
        """Walk up from script_path.parent, return first directory that contains a project marker."""
    ...
    def _run_script(
        ...
        effective_cwd = Path(working_dir) if working_dir else script_path.parent

        # Resolve script to an absolute path for project-root detection only.
        # cwd behaviour is unchanged: effective_cwd stays script_path.parent unless
        # working_dir is set explicitly.
        if script_path.is_absolute():
            resolved_script = script_path
        elif working_dir:
            resolved_script = Path(working_dir) / script_path
        else:
            resolved_script = Path.cwd() / script_path

        project_root = self._find_project_root(resolved_script)

        env = os.environ.copy()
        existing_pp = env.get("PYTHONPATH", "")
        parts = existing_pp.split(os.pathsep) if existing_pp else []

        to_prepend: list = []
        cwd_str = str(effective_cwd)
        if cwd_str not in parts:
            to_prepend.append(cwd_str)

        if project_root:
            root_str = str(project_root)
            if root_str not in parts and root_str not in to_prepend:
                to_prepend.append(root_str)

        if to_prepend:
            env["PYTHONPATH"] = os.pathsep.join(to_prepend + parts)
        ...
        self.logger.info(f"Arbeitsverzeichnis: {effective_cwd}")
        try:
